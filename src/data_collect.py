...
class DataCollector:
    def __init__(self):
        ...
        self.bag = rosbag.Bag('jackal_zed_test.bag', 'w')
	...
	print("sub_gps: %s", sub_gps)
        ...
        ts = message_filters.ApproximateTimeSynchronizer([sub_img, sub_imu, sub_odom, sub_odom_filtered, sub_odom_zed, sub_gps], queue_size=10, slop=0.5)
        ts.registerCallback(self.callback)

    def callback(self, img, imu, odom, odom_filtered, odom_zed, gps):
	...
	#print("imu.header: %s", imu.header)

        # Increase the sequence number
        self.seq += 1

        self.bag.write('/zed/zed_node/left/image_rect_color', img)
        self.bag.write('/imu/data', imu)
        self.bag.write('/jackal_velocity_controller/odom', odom)
        self.bag.write('/odometry/filtered', odom_filtered)
        self.bag.write('/zed/zed_node/odom', odom_zed)
